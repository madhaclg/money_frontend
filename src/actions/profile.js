...
export const loadProfile = () => async dispatch => {
    try {
        const res = await axios.get('https://moneytransfer-backend.onrender.com/api/allUsers')
        console.log(res);
        dispatch({
...
export const loadHistory = () => async dispatch => {
    try {
        const res = await axios.get('https://moneytransfer-backend.onrender.com/api/history')
        dispatch({
            type: HISTORY_LOADED,
            payload: res.data
        })
...
export const transfer = ({ From, To, amount }, history) => async dispatch => {
    ...
    try {
        const res = await axios.post('https://moneytransfer-backend.onrender.com/api/transaction', body, config)
        dispatch({
